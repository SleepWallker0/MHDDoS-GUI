const term = document.getElementById('terminal-content');
        ...
        const LOG_LEVELS = { 'DEBUG': 0, 'INFO': 1, 'SUCCESS': 2, 'WARNING': 3, 'ERROR': 4, 'SYSTEM': 5 };
        ...
        function setLogLevel(level) {
            ...
            if (typeof refreshLogVisibility === 'function') {
        ...
        function appendLog(data, forceLevel = null, taskId = null) {
            ...
            const msgRaw = typeof logData.msg === 'string' ? logData.msg : String(logData.msg || '');
            ...
            entry.style.display = (!matchesLevel || !matchesFilter || !matchesTask) ? 'none' : 'flex';
        ...
        function renderTasksList(tasks) {
            ...
            if (currentTasksJson === lastTasksJson && container.children.length > 0) {
                ...
                tasks.forEach(t => {
                    ...
                    const row = document.querySelector(`[data-task-row-id="${t.task_id}"]`);
                    if (row) {
                        const elapsedEl = row.querySelector('.task-elapsed');
                        const remainingEl = row.querySelector('.task-remaining');
                        const progressPctEl = row.querySelector('.task-progress-pct');
                        const progressBarEl = row.querySelector('.task-progress-bar');
                        
                        if (elapsedEl) elapsedEl.textContent = t.elapsed + 's';
                        if (remainingEl) remainingEl.textContent = formatDuration(timeLeft);
        ...
        function renderLoop(time) {
            ...
            try {
                ...
                console.error("UI Render Loop Exception:", e);
                // Attempt to notify terminal if possible
                try {
                    appendLog({ type: 'error', level: 'ERROR', time: new Date().toLocaleTimeString(), msg: `[UI CRASH PREVENTED] ${e.message}` });
                } catch (ignore) {}
            } finally {
                renderScheduled = false;
